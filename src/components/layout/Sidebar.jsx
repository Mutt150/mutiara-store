...
export default function Sidebar({ isSidebarOpen, isSidebarMini, setIsSidebarMini, activeTab, setActiveTab, setIsSidebarOpenState, handleLogout, navItems }) {
    return (
        <aside className={`fixed inset-y-0 left-0 z-50 bg-white border-r border-pink-100 transform transition-transform duration-300 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} ${isSidebarMini ? 'w-20' : 'w-80'} print:hidden flex flex-col h-full shadow-2xl md:shadow-none`}>
            ...
            <nav className={`space-y-1 overflow-y-auto custom-scrollbar flex-1 pb-10 ${isSidebarMini ? 'px-2' : 'px-6'}`}>
                {!isSidebarMini && <p className="px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-2">Utama</p>}
                ...
                {!isSidebarMini && <p className="px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-6">Gudang</p>}
                {navItems.inventory.map(item => (
                    <NavButton key={item.id} id={item.id} label={item.label} icon={item.icon} activeTab={activeTab} setActiveTab={setActiveTab} setIsSidebarOpen={setIsSidebarOpenState} isSidebarMini={isSidebarMini} />
                ))}

                {/* Tambahan Bagian Sistem & Log di Sidebar */}
                {!isSidebarMini && <p className="px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-6">Sistem</p>}
